	...
	<view class="market-container">
		...
		<swiper class="banner" indicator-dots autoplay circular>
			<swiper-item v-for="(item, index) in banners" :key="index">
				<image :src="item.image || item.url" mode="aspectFill" class="banner-image" @error="handleImageError($event, index)"></image>
			</swiper-item>
		</swiper>
		...
		<view class="quick-menu">
			<view class="menu-item" @tap="navigateTo('/pages/trade/trade')">
				<view class="menu-icon ri-icon">
					<text class="ri-icon-inner ri-funds-line"></text>
				</view>
				<text class="menu-text">量化介绍</text>
			</view>
			<view class="menu-item" @tap="navigateTo('/pages/exchange/exchange')">
				<view class="menu-icon ri-icon">
					<text class="ri-icon-inner ri-exchange-line"></text>
				</view>
				<text class="menu-text">交易所</text>
			</view>
			<view class="menu-item" @tap="$gopage('/pages/wallet/index')">
				<view class="menu-icon ri-icon">
					<text class="ri-icon-inner ri-wallet-3-line"></text>
				</view>
				<text class="menu-text">钱包</text>
			</view>
			<!-- <view class="menu-item" @tap="navigateTo('/pages/invite/invite')">
				<view class="menu-icon ri-icon">
					<text class="ri-icon-inner ri-user-add-line"></text>
				</view>
		...
		<view class="error-container" v-if="apiError">
			...
			<text class="error-title">获取数据失败</text>
			<text class="error-text">无法从CoinGecko获取市场数据，请确认您的网络连接正常。如果频繁出现此问题，这可能是由于直接API访问受限。</text>
			<button class="retry-button" @tap="refreshData">重试</button>
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			getMarketData() {
				...
				return api.getGlobalMarketData(10) // 使用10分钟缓存
				.then(data => {
			...
			handleImageError(event, index) {
				...
				event.target.src = '/static/images/banners/placeholder.png';
			}
		},
		// 下拉刷新
		onPullDownRefresh() {

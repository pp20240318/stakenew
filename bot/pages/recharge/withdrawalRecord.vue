<template>
	<view class="withdrawal-container">
		...
		<view class="navbar">
			...
			<text class="page-title">{{ $t('提现记录') }}</text>
		...
		<view class="records-content">
			...
			<view class="records-header">
				<text class="header-item">{{ $t('账号') }}</text>
				<text class="header-item">{{ $t('地址') }}</text>
			...
			<view class="records-list">
				<view class="record-item" v-for="(item, index) in datalist" :key="index">
					...
					<text class="record-data address-text">{{ item.deposit_address }}</text>
					<text class="record-data">{{ item.target_num }}</text>
				...
				<view v-if="datalist.length === 0" class="empty-state">
					<text class="empty-text">{{ $t('暂无提现记录') }}</text>
				</view>
			</view>
		</view>
	</view>

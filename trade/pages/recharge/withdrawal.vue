			...
			<view class="form-card">
				...
				<view class="form-item select-item">
					<select v-model="selecttype" @change="changetype()" class="select-control">
						<option value="1" class="option-item">USDT(TRC20)</option>
					</select>
				</view>
				...
				<!-- 支付密码 -->
				<view class="form-item">
					<input type="password" v-model="paypassword" :placeholder="$t('请输入支付密码')" class="input-control" />
				</view>

				<!-- 谷歌验证码（如果已绑定） -->
				<view class="form-item" v-if="googleAuthEnabled">
					<view class="google-auth-label">
						<text class="label-text">{{ $t('谷歌验证码') }}</text>
						<text class="label-required">*</text>
					</view>
					<input type="number" v-model="googleCode" maxlength="6" :placeholder="$t('请输入6位验证码')" class="input-control" />
				</view>
...
export default {
	...
	methods: {
			...
			t.req({
				...
				success: function(res) {
					if (res.code === 1 && res.data) {
						t.googleAuthEnabled = res.data.is_bound || false;
					}
				}
			});
		},
		...
		submit: function() {
			...
			// 如果启用了谷歌验证器，验证验证码
			if (t.googleAuthEnabled) {
				if (!t.googleCode || t.googleCode.length !== 6) {
			...
			t.req({
				...
				data: {
					...
					bankname: t.bankname,
					banktype: t.banktype,
					bankdname: t.bankdname,
					bankcode: t.bankcode,
					paypassword: t.paypassword,
					userid: userid,
					show_percent: t.show_percent,
					google_code: t.googleCode
				},
				success: function(i) {
					if (i.code == 0) {
